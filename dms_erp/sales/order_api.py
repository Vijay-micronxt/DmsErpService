...
def _create_order(
	dealer: str, lines: list[dict], expected_dispatch, inquiry: str, channel: str | None = None, customer_po: str | None = None
) -> dict:
	...
	the native Sales Order.po_no and closes the source Inquiry (BRD C.3.1)."""
	if not lines:
...
def create_order(dealer: str, lines: list[dict], expected_dispatch, inquiry: str, channel: str | None = None):
	...
	return _create_order(dealer, lines, expected_dispatch, inquiry, channel)
