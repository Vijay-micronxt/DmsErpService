...
| Method | Role | Notes |
|---|---|---|
| `get_dashboard` | any of the four DMS roles, or System Manager | role-agnostic entry point — resolves the caller's own primary role (same precedence as `auth.api.login`'s `user.primary_role`) and returns `{"role", "widgets"}`: the Number Card/Dashboard Chart widgets on that role's ERPNext-native `Dashboard` doc (`DMS Sales`/`DMS Purchase`/`DMS Warehouse`/`DMS Management` — same name as the DMS Role doc itself; built by a System Manager in the desk UI, no code change needed to add/remove a widget). Returns an empty `widgets` list, not an error, until that Dashboard doc is configured. Does **not** call the four functions below. |
