...
| Method | Role | Notes |
|---|---|---|
| `get_dashboard` | any of the four DMS roles, or System Manager | role-agnostic entry point — resolves every DMS role the caller holds (same list/order as `auth.api.login`'s `user.app_roles`) and returns `{"dashboards": [{"role", "widgets"}, ...]}`, one entry per role (a single-role user still gets a one-entry array): the Number Card/Dashboard Chart widgets on that role's ERPNext-native `Dashboard` doc (`DMS Sales`/`DMS Purchase`/`DMS Warehouse`/`DMS Management` — same name as the DMS Role doc itself; built by a System Manager in the desk UI, no code change needed to add/remove a widget). An entry's `widgets` is `[]`, not an error, until that role's Dashboard doc is configured. Does **not** call the four functions below. |
