...
| `series_ref` | string | **required** | Series master name (BRD C.1.1) — `finish`/`pieces_per_box`/`sqft_per_box`/`weight_per_box_kg` fall back to the Series' own values for any of those left unset here (explicit params always win); `bulkQtyThreshold`/`retailQtyThreshold` and the `series` display label always come from the Series, with no caller override (matches ERPNext's own "fetch_from" convention — a link-derived label can't be set independently, so it can't drift out of sync with the master it's linked to). Enforced on creation only — a handful of items created before this rule existed have no Series attached, and `update_product` still allows leaving it unset when editing one of those. |
| `size, finish, color, swatch` | string | optional |  |
...
|---|---|---|---|
| `item` | string | required | Item code |
| `patch` | object | required | keys: name, category, size, finish, color, swatch, status, piecesPerBox, sqftPerBox, weightPerBoxKg, leadTimeDays, altItemId, hsnCode, seriesRef, bulkQtyThreshold, retailQtyThreshold, dealerCodes, images. `series` (the display label) isn't a settable key — like `create_product`, it's always re-derived from `seriesRef` whenever that key is present in the patch, never independently editable. |
...
**Role-agnostic entry point** — resolves every DMS role the caller holds (the same list, in the same highest-privilege-first order, as `auth.api.login`'s `user.app_roles`) and returns one **ERPNext-native** `Dashboard` doc's widgets per role, so the frontend never has to know which of `sales_dashboard`/`purchase_dashboard`/`warehouse_dashboard`/`management_dashboard` to call, and an admin can add/remove/reorder a KPI card or chart from the desk UI with no code change or deploy. A user holding only one DMS role gets back a single-entry list — there's one response shape for every caller. A System Manager-only account (no `DMS *` role — the admin escape hatch documented in `auth/api.py`) resolves to a single `"management"` entry.
...
...then adds `Number Card`/`Dashboard Chart` widgets to it the normal ERPNext way. Until that Dashboard doc exists, `get_dashboard` returns an empty `widgets` list for that entry rather than erroring — this endpoint ships ahead of that configuration being done.
  ...
  "dashboards": [
    {
      "role": "management",
      "widgets": [
        { "type": "number_card", "name": "Total Sales MTD", "label": "Total Sales MTD", "value": 4820000, "color": "#29CD42", "trend": null }
      ]
    },
    {
      "role": "sales",
...
`dashboards` has one entry per DMS role the caller holds, highest-privilege first (Management, Purchase, Warehouse, Sales) — a single-role user still gets a one-entry array, so there's exactly one response shape to handle. Each widget's `type` is either `"number_card"` (with `value`, optional `trend` percentage) or `"chart"` (with `chart_type`: `line`/`bar`/`percentage`/`pie`/`donut`/`heatmap`, and either `labels`+`datasets`, or for heatmap charts `labels: []`+`dataPoints`). Widgets are permission-filtered individually by Frappe's own `Dashboard.get_permitted_cards`/`get_permitted_charts` — one scoped to a doctype/report the caller can't read is silently omitted, not surfaced as an error. The four `sales_dashboard`/`purchase_dashboard`/`warehouse_dashboard`/`management_dashboard` endpoints below are unaffected and still independently callable/whitelisted, but `get_dashboard` no longer routes through them.
